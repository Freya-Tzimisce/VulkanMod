...
public abstract class AbstractBlockRenderContext extends AbstractRenderContext {
	...
	protected final Object2ByteLinkedOpenHashMap<ShapePairKey> occlusionCache = new Object2ByteLinkedOpenHashMap<ShapePairKey>(2048, 0.25F) {
	...
	protected abstract VertexConsumer getVertexConsumer(RenderType renderType);
	...
	protected RenderType effectiveRenderType(BlendMode blendMode) {
	...
	public void emitVanillaBlockQuads(BlockStateModel model, @Nullable BlockState state, Supplier<RandomSource> randomSupplier, Predicate<Direction> cullTest) {
		...
		RenderMaterial defaultMaterial = state.getLightEmission() == 0 ? STANDARD_MATERIAL : NO_AO_MATERIAL;
		...
		for (int i = 0; i <= ModelHelper.NULL_FACE_ID; i++) {
			...
			final List<BlockModelPart> parts = ((BlockStateModel)this).collectParts(this.random);
