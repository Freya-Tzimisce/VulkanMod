...
public class AttributeParser {
	...
	public AttributeParser(GlslConverter converterInstance, VertexFormat vertexFormat) {
	...
	public boolean parseToken(String token) {
		if (this.ioType == null) {
			...
			token = UniformParser.removeSemicolon(token);
			...
			if (this.shaderStage == GlslConverter.ShaderStage.Vertex) {
				switch (this.ioType) {
					case "in" -> {
						int i = this.vertexFormat.getElementAttributeNames().indexOf(this.name);

						if (i == -1) {
							throw new IllegalStateException("Element not found");
						}

						this.vertInAttributes.add(i, this.type, this.name);
						}
	...
	static class AttributeSet {
		...
		void add(int location, String type, String name) {
			this.attributes.add(new Attribute(location, type, name));
		}
		...
		boolean contains(String type, String name) {
			return this.attributes.stream()
					.anyMatch(attribute -> Objects.equals(attribute.name, name) && Objects.equals(attribute.type, type));
		}
	}
}
